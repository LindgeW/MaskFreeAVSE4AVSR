...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, x):  # (B, L, D)
        return F.dropout(x + self.pe[:, :x.size(1)].detach(), p=self.dropout, training=self.training)
...
class TransDecoder(nn.Module):
    ...
    def forward(self, tgt, src_enc, tgt_lens=None, src_lens=None):
        tgt_embed = self.tok_embedding(tgt) * (self.d_model ** 0.5)
...
class CTCLipModel(nn.Module):
    def __init__(self, vocab_size, se=False):
        ...
        self.av_trans = TransformerEncoder(512, 4, 3, attn_mask=False)
        ...
        #self.cfm_c = Conformer(256, 4, 256 * 4, 3, 31, 0.1)
        ...
        # self.fc_a = nn.Linear(256, vocab_size - 1)  # including blank label, excluding bos
        # self.fc_v = nn.Linear(256, vocab_size - 1)  # including blank label, excluding bos
    ...
    def init_forward(self, vid_inp, aud_inp, tgts, vid_lens=None, aud_lens=None, tgt_lens=None):
        ...
        p = torch.rand(1).item()
        if p < 0.25:
            av = self.av_trans((enc_a + ea).transpose(0, 1)).transpose(0, 1)
            src_lens = aud_lens
        elif p < 0.5:
            av = self.av_trans((enc_v + ev).transpose(0, 1)).transpose(0, 1)
            src_lens = vid_lens
        else:
        ...
        attn_loss = F.cross_entropy(dec_out.transpose(-1, -2).contiguous(), tgts[:, 1:], ignore_index=0)
        avsr_loss = 0.9 * attn_loss + 0.1 * ctc_loss
        return avsr_loss

    def forward(self, vid_inp, aud_inp, tgts, vid_lens=None, aud_lens=None, tgt_lens=None):  # (b, t, c, h, w)
        '''
        p = torch.rand(1).item()
        if p < 0.1:
        ...
        av, gating_logits = self.router(enc_v.detach(), enc_a.detach(), hv, ha, av)
...
class DRLModel(nn.Module):
    ...
    def forward(self, vids, auds, tgts, vid_lens, aud_lens, tgt_lens):   # (B, T, C, H, W)
        avsr_loss, drl_loss = self.avsr(vids, auds, tgts, vid_lens, aud_lens, tgt_lens)
