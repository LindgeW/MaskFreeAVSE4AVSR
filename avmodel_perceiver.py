...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, x):
        if self.batch_first:  # (B, L, D)
            return F.dropout(x + self.pe[:, :x.size(1)], p=self.dropout, training=self.training)
        else:  # (L, B, D)
            return F.dropout(x + self.pe[:, :x.size(0)].transpose(0, 1), p=self.dropout, training=self.training)
...
class DynamicRouter(nn.Module):
    ...
    def routing(self, v, a):
        ...
        #sv = v.mean(dim=1) + v.std(dim=1) + torch.max(v, dim=1)[0]
        #sa = a.mean(dim=1) + a.std(dim=1) + torch.max(a, dim=1)[0]
...
class CTCLipModel(nn.Module):
    def __init__(self, vocab_size, se=False):
        ...
        for par in self.fsns:
            nn.init.trunc_normal_(par, 0, 0.02)
        ...
        # for mbt
        self.a_mbt_attns = nn.ModuleList([Attention(512, 4, 0.1, batch_first=False, is_cross_attn=False, norm_first=True) for _ in range(4)])
        self.v_mbt_attns = nn.ModuleList([Attention(512, 4, 0.1, batch_first=False, is_cross_attn=False, norm_first=True) for _ in range(4)])
        #self.av_mbt_attns = nn.ModuleList([Attention(512, 4, 0.1, batch_first=False, is_cross_attn=False, norm_first=True) for _ in range(4)])
        self.post_norm1 = nn.LayerNorm(512)
        self.post_norm2 = nn.LayerNorm(512) 
        self.post_norm3 = nn.LayerNorm(512) 
    ...
    def forward(self, vid_inp, aud_inp, clean_aud_inp, tgts, vid_lens=None, aud_lens=None, tgt_lens=None):  # (b, t, c, h, w)   (b, t, c)
        ...
        # MBT
        #enc_v = self.score_vid(enc_v)
        #enc_a = self.score_aud(enc_a)
        fsn = self.fsn.unsqueeze(0).repeat(vid.shape[0], 1, 1).transpose(0, 1)   # (B, N, D) -> (N, B, D)
        L = fsn.shape[0] 
        for i, (v_mbt, a_mbt) in enumerate(zip(self.v_mbt_attns, self.a_mbt_attns)):
        #for v_mbt, a_mbt in zip(self.av_mbt_attns, self.av_mbt_attns):
        ...
        #    fsn = fsn.unsqueeze(0).repeat(vid.shape[0], 1, 1).transpose(0, 1)   # (B, N, D) -> (N, B, D)
        #    fsn_v = v_mbt(torch.cat([fsn, enc_v], dim=0))
        #    fsn, enc_v = fsn_v[:fsn.shape[0]], fsn_v[fsn.shape[0]:]
        #    fsn_a = a_mbt(torch.cat([fsn, enc_a], dim=0))
        #    fsn, enc_a = fsn_a[:fsn.shape[0]], fsn_a[fsn.shape[0]:]
        
        av = self.post_norm1(enc_a).transpose(0, 1)   # (B, T, D)
        va = self.post_norm2(enc_v).transpose(0, 1)
        #w = self.sigmoid(self.balance)
        #av = w * av + (1 - w) * va
        
        ra = self.post_norm3(enc_a).transpose(0, 1)   # (B, T, D)
        #recon_a = self.aud_decoder(ra.transpose(1, 2)).transpose(1, 2)
        recon_a = self.aud_decoder(ra.transpose(1, 2)).reshape(ra.shape[0], -1, 4, ra.shape[1]).flatten(2).transpose(1, 2)
        recon_loss = 0.5 * F.l1_loss(recon_a, clean_aud_inp.detach())
        
        '''
        # Perceiver-MBT
        latents = self.latents.unsqueeze(0).repeat(vid.shape[0], 1, 1).transpose(0, 1)   # (B, N, D) -> (N, B, D)
       ...
       #orth_sim_loss = ((diff_loss(z_a, non_z_a) + diff_loss(z_v, non_z_v) + diff_loss(non_z_a.mean(dim=1), non_z_v.mean(dim=1))) / 3. +
        ...
        #ctc_loss = F.ctc_loss(ctc_log_probs, tgts[:, 1:], src_lens.reshape(-1), tgt_lens.reshape(-1), zero_infinity=True)
        dec_out = self.trans_dec(tgts[:, :-1], av, tgt_lens, src_lens)
    ...
    def beam_search_decode(self, vid_inp, aud_inp, vid_lens, aud_lens, bos_id, eos_id, max_dec_len=50, pad_id=0):
        with torch.no_grad():
            ...
            #av = self.cross_v(self.latents.repeat(vid.shape[0], 1, 1).transpose(0, 1), vid.transpose(0, 1), vid.transpose(0, 1))
            #av = self.self_v(av)
            ...
            av = self.post_norm1(enc_a).transpose(0, 1)
            #va = self.post_norm2(enc_v).transpose(0, 1)
            #w = self.sigmoid(self.balance)
            #av = w * av + (1 - w) * va
            ...
            res = beam_decode(self.trans_dec, av, src_lens, bos_id, eos_id,
