...
from custom_multihead_attention import Attention
...
class PositionalEncoding(nn.Module):
    def __init__(self, d_model, max_len=1000, dropout=0.1, batch_first=True):
...
class PixelShuffle1D(nn.Module):
    ...
    def forward(self, x):
        ...
        B, C_r, L = x.size()
        C = C_r // self.factor
        # [B, C*r, L] → [B, C, r, L] → [B, C, L, r] → [B, C, L*r]
        x = x.reshape(B, C, self.factor, L)
        x = x.permute(0, 1, 3, 2)  # 交换r和L维度
        x = x.reshape(B, C, L * self.factor)
        return x


class ConformerBlock(nn.Module):
    def __init__(self, dim, num_heads, kernel_size=31, num_bottleneck_tokens=4, dropout=0.1):
        super().__init__()
        self.dim = dim
    ...
    def forward(self, x, bottleneck_tokens):  # (B, L, D)   (B, K, D)
        combined_input = torch.cat([x, bottleneck_tokens], dim=1)   # (B, L+K, D)

        attn_input = 0.5 * self.ffn1(combined_input) + combined_input
        
        norm_attn_input = self.attn_layer_norm(attn_input)
        attn_output = self.attention(norm_attn_input, norm_attn_input, norm_attn_input, need_weights=False)[0] + attn_input   # (B, L+K, D)

        modality_out = attn_output[:, :-self.num_bottleneck_tokens]   # (B, L, D)
        bottleneck_out = attn_output[:, -self.num_bottleneck_tokens:]  # (B, K, D)
...
class ConformerMBT(nn.Module):
    def __init__(self, dim=256, num_heads=4, kernel_size=31, num_layers=4, num_bottleneck_tokens=4, dropout=0.1):
        super().__init__()
        self.bottleneck_tokens = nn.Parameter(torch.randn(num_bottleneck_tokens, dim))
        nn.init.trunc_normal_(self.bottleneck_tokens, 0, 0.02)
        
        self.audio_layers = nn.ModuleList([
            ConformerBlock(dim, num_heads, kernel_size, num_bottleneck_tokens, dropout)
    ...
    def forward(self, audio, video):
        # audio: (B, La, D)   video: (B, Lv, D)
        B = audio.size(0)
        bottleneck_tokens = self.bottleneck_tokens.unsqueeze(0).repeat(B, 1, 1)
        for audio_layer, video_layer in zip(self.audio_layers, self.video_layers):
            #video, bottleneck_video = video_layer(video, bottleneck_tokens)
            #audio, bottleneck_audio = audio_layer(audio, bottleneck_tokens)
            #bottleneck_tokens = (bottleneck_audio + bottleneck_video) / 2
            video, bottleneck_tokens = video_layer(video, bottleneck_tokens)
            audio, bottleneck_tokens = audio_layer(audio, bottleneck_tokens)
        return audio, video
...
class CTCLipModel(nn.Module):
    def __init__(self, vocab_size, se=False):
        ...
        self.av_mbt = ConformerMBT(dim=512, num_heads=4, kernel_size=31, num_layers=3, num_bottleneck_tokens=4)
        ...
        self.aud_decoder = nn.Sequential(
            ...
            #nn.ConvTranspose1d(256, 80, kernel_size=3, stride=2, padding=1, output_padding=1, bias=False),
            nn.BatchNorm1d(80),
            nn.LeakyReLU(),
            nn.Conv1d(80, 80, kernel_size=1, stride=1, padding=0, bias=False),
            #nn.Conv1d(80, 80, kernel_size=3, stride=1, padding=1, bias=False),
            nn.BatchNorm1d(80),
        )
        '''
       
        self.aud_decoder = nn.Sequential(  # 参数更少，计算高效，无棋盘格伪影, 全局建模, 限制：必须是整数倍变换
            nn.Conv1d(512, 160*2, kernel_size=3, stride=1, padding=1, bias=False),
            nn.BatchNorm1d(160*2),
            PixelShuffle1D(2),   
            nn.LeakyReLU(),
            nn.Conv1d(160, 80*2, kernel_size=3, stride=1, padding=1, bias=False),
            nn.BatchNorm1d(80*2),
            PixelShuffle1D(2),   
            nn.LeakyReLU(),
            nn.Conv1d(80, 80, kernel_size=1, stride=1, padding=0, bias=False),   # 1x1
            #nn.Conv1d(80, 80, kernel_size=3, stride=1, padding=1, bias=False),
            nn.BatchNorm1d(80)   
        )   # (*, Cxr, L) -> (*, C, Lxr)

        #self.whisper_model = whisper.load_model('small')  # 80
    ...
    def forward(self, vid_inp, aud_inp, clean_aud_inp, tgts, vid_lens=None, aud_lens=None, tgt_lens=None):  # (b, t, c, h, w)   (b, t, c)
        '''ck_run
        p = torch.rand(1).item()
        if p < 0.1:
        ...
        '''
        res_v, res_a = enc_v, enc_a
        # clean audio reconstruction
        fsn = self.fsn.unsqueeze(0).repeat(bs, 1, 1)   # (B, N, D) 
        L = fsn.shape[1] 
        #enc_a, enc_v = self.pos_enc(enc_a), self.pos_enc(enc_v)   
        ...
        '''
        enc_a, enc_v = self.av_mbt(enc_a, enc_v)
        recon_aud = self.aud_decoder(enc_a.transpose(1, 2)).transpose(1, 2)        

        #ra = self.post_norm(enc_a)   # (B, T, D)
        #ra = self.post_norms[0](enc_a)   # (B, T, D)
        #recon_aud = self.aud_decoder(ra.transpose(1, 2)).transpose(1, 2)        
        #recon_aud = self.aud_decoder(ra.transpose(1, 2)).reshape(ra.shape[0], -1, 4, ra.shape[1]).flatten(2).transpose(1, 2)
        
        drl_loss = 0.1 * F.l1_loss(recon_aud, clean_aud_inp.detach()) + 0.1 * F.mse_loss(self.audio_frontend(recon_aud), self.audio_frontend(clean_aud_inp.detach()))
        #w = torch.sigmoid(torch.norm(clean_aud_inp - aud_inp, p=2, dim=(-1, -2)) / torch.norm(clean_aud_inp, p=2, dim=(-1, -2))) - 0.5    # [0, 0.5)
        #w = torch.norm(clean_aud_inp - aud_inp, p=2, dim=(-1, -2)) / (torch.norm(clean_aud_inp, p=2, dim=(-1, -2)) + torch.norm(aud_inp, p=2, dim=(-1, -2)))   # [0, 1)
        #drl_loss = torch.mean(w.detach() * (F.l1_loss(recon_aud, clean_aud_inp.detach(), reduction='none').mean(dim=(-1, -2)) + F.mse_loss(self.audio_frontend(recon_aud), self.audio_frontend(clean_aud_inp.detach()), reduction='none').mean(dim=(-1, -2))))
        #print(w) 
        
        #with torch.no_grad():
        #    pad_clean_aud = F.pad(clean_aud_inp, pad=(0, 0, 0, 3000-clean_aud_inp.shape[1]))  # (B, 3000, F)
        #    clean_feat = self.whisper_model.embed_audio(pad_clean_aud.transpose(1, 2))   # (B, F, T)
        #pad_recon_aud = F.pad(recon_aud, pad=(0, 0, 0, 3000-recon_aud.shape[1]))
        #recon_feat = self.whisper_model.embed_audio(pad_recon_aud.transpose(1, 2))
        #drl_loss = 0.1 * F.l1_loss(recon_aud, clean_aud_inp.detach()) + 0.1 * F.mse_loss(recon_feat, clean_feat)
        ...
        #enc_av = self.cfm_c(torch.cat([enc_a + res_a, enc_v + res_v], dim=1), enc_a.shape[1]+vid_lens)
        av = enc_av[:, :enc_a.shape[1]]
        va = enc_av[:, enc_a.shape[1]:]
       ...
       #orth_sim_loss = ((diff_loss(z_a, non_z_a) + diff_loss(z_v, non_z_v) + diff_loss(non_z_a.mean(dim=1), non_z_v.mean(dim=1))) / 3. +
        ...
        ctc_loss = F.ctc_loss(self.fc_ctc(av).log_softmax(dim=-1).transpose(0, 1), tgts[:, 1:], aud_lens.reshape(-1), tgt_lens.reshape(-1), zero_infinity=True) + F.ctc_loss(self.fc_ctc(va).log_softmax(dim=-1).transpose(0, 1), tgts[:, 1:], vid_lens.reshape(-1), tgt_lens.reshape(-1), zero_infinity=True)
        #attn_loss = F.cross_entropy(self.trans_dec(tgts[:, :-1], av, tgt_lens, aud_lens).transpose(-1, -2), tgts[:, 1:], ignore_index=0) + F.cross_entropy(self.trans_dec(tgts[:, :-1], va, tgt_lens, vid_lens).transpose(-1, -2), tgts[:, 1:], ignore_index=0)
    ...
    def beam_search_decode(self, vid_inp, aud_inp, vid_lens, aud_lens, bos_id, eos_id, max_dec_len=50, pad_id=0):
        with torch.no_grad():
            ...
            # recycle 
            #recon_aud = self.aud_decoder(enc_a.transpose(1, 2)).transpose(1, 2)        
            #aud = self.audio_frontend(recon_aud)
            #enc_a = self.cfm_a(aud, aud_lens)
            #enc_a, enc_v = self.av_mbt(enc_a, enc_v)
            ...
            res = beam_decode(self.trans_dec, av, aud_lens, bos_id, eos_id, max_output_length=max_dec_len, beam_size=40)
