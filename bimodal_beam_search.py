...
def tile(x: Tensor, count: int, dim=0):
    """
...
def subsequent_mask(size):
    ...
    subseq = np.triu(np.ones(attn_shape), k=1).astype('uint8')
    return torch.from_numpy(subseq) == 0


def beam_decode(
        decoder,
        audio_encoder_output: Tensor,
        video_encoder_output: Tensor,
        #src_mask: Tensor,
        aud_lens: Tensor,
        vid_lens: Tensor,
        bos_index: int,
    ...
    batch_size = len(aud_lens)
    DEVICE = audio_encoder_output.device

    audio_encoder_output = tile(
        audio_encoder_output.contiguous(), beam_size, dim=0
    )  # batch*k x src_len x enc_hidden_size
    video_encoder_output = tile(
        video_encoder_output.contiguous(), beam_size, dim=0
    )  # batch*k x src_len x enc_hidden_size

    #src_mask = tile(src_mask, beam_size, dim=0)  # batch*k x 1 x src_len
    aud_lens = tile(aud_lens, beam_size, dim=0)
    vid_lens = tile(vid_lens, beam_size, dim=0)
    ...
    for step in range(max_output_length):
        ...
        tgt_lens = (decoder_input > 0).sum(dim=-1)
        logits = decoder(decoder_input, audio_encoder_output, video_encoder_output, tgt_lens, aud_lens, vid_lens)
        #logits = decoder(decoder_input, encoder_output, src_mask)[0]
        # subsequent_mask(decoder_input.size(1)).to(DEVICE).long())
        ...
        curr_scores = curr_scores.reshape(-1, beam_size * output_size)
        ...
        if is_finished.any():
            ...
            for i in range(is_finished.size(0)):
                ...
                if end_condition[i]:
                    best_hyp = sorted(hypotheses[b], key=lambda x: x[0], reverse=True)  # 降序
        ...
        select_indices = batch_index.view(-1)
        audio_encoder_output = audio_encoder_output.index_select(0, select_indices)
        video_encoder_output = video_encoder_output.index_select(0, select_indices)
        #src_mask = src_mask.index_select(0, select_indices)
        aud_lens = aud_lens.index_select(0, select_indices)
        vid_lens = vid_lens.index_select(0, select_indices)
