...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, x):  # (B, L, D)
        ...
        return F.dropout(self.embed_scale * x + self.pe[:, :x.size(1)].detach(), p=self.dropout, training=self.training)
...
class MultiheadAttention(nn.Module):
    ...
    """
    def __init__(self, embed_dim, num_heads, attn_dropout=0.1, bias=True, add_bias_kv=False, add_zero_attn=False):
        super().__init__()
...
class TransformerEncoder(nn.Module):
    ...
    """
    def __init__(self, embed_dim, num_heads, num_layers, attn_dropout=0.1, relu_dropout=0.1, res_dropout=0.1, embed_dropout=0.2, attn_mask=False):
        super().__init__()
