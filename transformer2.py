...
# https://github.com/mdswyz/DMD/blob/main/trains/subNets/transformers_encoder/transformer.py
...
class TransformerEncoder(nn.Module):
    ...
    """
    def __init__(self, embed_dim, num_heads, num_layers, attn_dropout=0.1, relu_dropout=0.1, res_dropout=0.1, embed_dropout=0.2, attn_mask=False, need_pos_enc=True):
        super().__init__()
    ...
    def forward(self, x_in, x_in_k=None, x_in_v=None):
        ...
        if self.embed_positions is not None and self.need_pos_enc:
...
def LayerNorm(embedding_dim, eps=1e-5):
    m = nn.LayerNorm(embedding_dim, eps=eps)
    return m
